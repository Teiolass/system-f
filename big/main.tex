...
\]
\deb{[controlla che sia giusto]} rappresenta la funzione $f$ definita con $f(0)
= n$ e $f(k+1) = h(f(k),k)$. \deb{Dimostro?}
...
per tale studio:
\begin{block}[Definizione]
    \deb{Data una teoria $T$ nel linguaggio dell'aritmetica, una funzione
...
Estentendiamo i tipi del sistema T con un nuovo tipo $\one$ e con un termine
$\i$ di tipo $\one$. Per ogni tipo $U$ di questo sistema esteso definiamo un
tipo $|U|$ che sia o uguale a $\one$ o tale che $\one$ non vi compaia. Il tipo
$|U|$ \`e defitnito induttivamente con le seguenti regole:
\begin{itemize}
...
ristretta ai soli numeri naturali si scriverebbe come $\forall x\ (\text{int}(x)
\rar \phi)$. In modo equivalente la formula $\exists x\ \phi$ si riscriverebbe
come $\exists x\ (\text{int}(x) \land \phi$.
...
\begin{proof}
    ...
    che il termine $\l x. \pi^1 (Mx)$ rappresenta $f$, infatti \deb{$\phi(
    \overline{n}, \overline{m})$} \`e vera, e quindi $f(n)=m$.
\end{proof}
...
\end{itemize}
Notiamo che tuttavia non \`e possibile esprimere la riducibilit\`a in modo
uniforme su tutti i tipi del sistema F, ma soltanto caso per caso per ciascun
tipo.
...
l'assiociare ad ogni formula $\phi$ in HA2 (dimostrabile o non dimostrabile) un
tipo $\cod{\phi}$ del sistema F. L'assiociazione viene fatta con le seguenti
regole:
\begin{itemize}
...
Ora possiamo definire un traduzione di ogni dimostrazione di una formula $\phi$
...
\begin{itemize}
    ...
    \item Ciascun assioma viene tradotto in un qualunque termine fissato del
        tipo corretto.
