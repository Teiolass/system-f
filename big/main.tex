...
La propriet\`a di essere in forma normale restringe la possibile struttura in
cui possono essere i termini del $\l$-calcolo.
...
termini tipato semplice un sottoinsieme), \`e possibile definire le nozioni di
...
scelta del tipo per le variabili che vi compaiono non \`e unica. Infatti per
ogni tipo $U$ possiamo costruire per ogni naturale $n$ il corrispondente
numerale
\[
...
molto bassa.  Per ovviare a questo problema introduciamo un nuovo calcolo, il
sistema T di G\"odel, in cui vengono artificialmente inseriti tipi per gli
interi e alcuni termini che rappresentano delle funzioni basilari su di essi.
Vedremo che con queste semplici aggiunte il calcolo guadagna una forte potenza
espressiva.
...
Inoltre nel sistema T \`e tipabile anche l'esponenziazone, con il termine
\[
...
primi due argomenti sono sempre di tipo $\tint$, $\tint\rar\tint\rar\tint$.
...
che $\one$ non vi compaia. Il tipo $|U|$ \`e defitnito induttivamente con le
seguenti regole:
\begin{itemize}
...
riscriverebbe come $\exists x\ (\text{int}(x) \land \phi$.
...
\begin{proof}
    ...
    proprosizione relativamente al caso intuizionista. Sia allora $f$ una
...
Inoltre, nell'ambito della logica intuizionista, anche il $\bot$ \`e superfluo e
pu\`o essere riscritto come $\bot = \forall X.X$.
...
\begin{block}[Definizione]
    ...
    ottenuta sostituendo ogn sottoformula atomica (compresa $\bot$ quindi)
    $\eta$ presente in $\phi$ con $\eta\lor\psi$, assumendo che le variabili
    libere di $\eta$ non siano legate in $\phi$.
\end{block}

Risulta utile verificare le seguenti proprit\`a per il costrutto appena
definito.
\begin{block}[Lemma]
...
\end{itemize}
Notiamo che tuttavia non \`e nuovamente possibile esprimere la riducibilit\`a in
modo uniforme su tutti i tipi del sistema F, ma soltanto caso per caso per
ciascun tipo.
...
\begin{proof}
    ...
    da dimostrare \`e atomica. Continuando a ragionare in questo modo si ha il
    risultato.
\end{proof}
...
\begin{proof}
    ...
    $\phi(S^nO, S^mO)$ \`e dimostrabile in HA2, \deb{essa \`e vera nel modello
